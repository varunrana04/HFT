...
```
Usage: python mt5_gateway.py [OPTIONS]

Options:
    --symbol STR        MT5 symbol (default: EURUSD)
    --lot FLOAT         Lot size (default: 0.01)
    --magic INT         Magic number (default: 42000)
    --duration INT      Trading duration in seconds (default: 3600)
    --threshold FLOAT   Signal threshold (default: 0.0005)
    --log PATH          Trade log CSV path (default: results/mt5_trades.csv)
...
```
Usage: python tv_webhook.py [OPTIONS]

Options:
    --port INT              Server port (default: 8080)
    --host STR              Server host (default: 0.0.0.0)
    --broker {sim,alpaca}   Broker backend (default: sim)
    --qty FLOAT             Default order quantity (default: 0.01)
    --max-position FLOAT    Max position size (default: 10.0)
    --max-daily-loss FLOAT  Max daily loss in $ (default: 500)

Endpoints:
    POST /webhook           TradingView alert receiver
    GET  /health            Server health + position
    GET  /trades            All executed trades
    POST /reset             Reset daily risk counters
...
### 13.4 `data_downloader.py`

```
Usage: python data_downloader.py [OPTIONS]

Downloads historical trade data from Binance REST API.
Supports aggTrades and klines. Saves to CSV/Parquet.

Options:
    --symbol STR        Trading pair (default: BTCUSDT)
    --type STR          Data type: aggTrades | klines (default: aggTrades)
    --start DATE        Start date YYYY-MM-DD
    --end DATE          End date YYYY-MM-DD
    --output DIR        Output directory (default: data)
    --format STR        Output format: csv | parquet (default: csv)
...
# Run backtest
python python/backtest.py --data data/BTCUSDT_trades.csv

# Start MT5 gateway
python python/mt5_gateway.py --symbol EURUSD --lot 0.01

# Start TradingView webhook
python python/tv_webhook.py --port 8080
```
