...
TEST(Features, MicropriceSym) {
    ...
    double raw = engine.compute_microprice(book);
    ASSERT_TRUE(std::abs(raw - expected_mid) < 0.01);
    return true;
}
...
TEST(Features, MicropriceAsym) {
    ...
    double raw = engine.compute_microprice(book);
    ASSERT_TRUE(std::abs(raw - expected) < 0.01);
    return true;
}
...
TEST(Features, OFIBidIncrease) {
    ...
    double raw = engine.compute_ofi(book2);

    // OFI = (20-10) - (10-10) = 10
    ASSERT_TRUE(std::abs(raw - 10.0) < 0.01);
    return true;
}
...
TEST(Features, OFIAskDecrease) {
    ...
    // OFI = 0 - (5-20) = 15
    ASSERT_TRUE(std::abs(raw - 15.0) < 0.01);
    return true;
}
...
TEST(Features, SpreadBPS) {
    ...
    double raw = engine.compute_spread_bps(book);
    ASSERT_TRUE(std::abs(raw - expected_bps) < 0.1);
    return true;
}
...
TEST(Features, SpreadBPSTight) {
    ...
    double expected_bps = 0.01 / 50000.005 * 10000.0;
    double raw = engine.compute_spread_bps(book);
    ASSERT_TRUE(std::abs(raw - expected_bps) < 0.1);
    return true;
}
...
TEST(Features, InvalidBook) {
    hft::FeatureConfig cfg;
    cfg.normalizer_min_obs = 1;
    hft::FeatureEngine engine(cfg);
    hft::BookSnapshot book{};
    std::memset(&book, 0, sizeof(book));
